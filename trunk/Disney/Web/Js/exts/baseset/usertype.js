usertype = function(node) {
    ...
    var url = '';
    var roleCmbStore = new Ext.data.Store({
        idProperty: 'ID',
        reader: new Ext.data.JsonReader({ fields: Ext.data.Record.create(['ID', 'RoleName']) }),
        url: '/sys/role'
    });
    roleCmbStore.load();
    var roleCmb = new Ext.form.ComboBox({ mode: 'local', triggerAction: 'all', forceSelection: true, editable: false,
        allowBlank: false, emptyText: '---请选择---', fieldLabel: '角色', name: 'RoleID',
        hiddenName: 'RoleID', displayField: 'RoleName', valueField: 'ID', width: 120, store: roleCmbStore
    });
    var typeCmbStore = new Ext.data.Store({
        reader: new Ext.data.JsonReader({ fields: Ext.data.Record.create([{ name: 'displayValue' }, { name: 'displayText'}]), root: "data" }),
        url: '/baseset/usertypes'
    });
    typeCmbStore.load();
    var typeCmb = new Ext.form.ComboBox({ mode: 'local', triggerAction: 'all', forceSelection: true, editable: false,
        allowBlank: false, emptyText: '---请选择---', fieldLabel: '类型', name: 'Type',
        hiddenName: 'Type', displayField: 'displayText', valueField: 'displayValue', width: 120, store: typeCmbStore
    });
    ...
    var store = new Ext.data.Store({
        reader: new Ext.data.JsonReader({ fields: dataType, root: "data" }),
        url: '/baseset/usertype'
    });
    ...
    var cm = new Ext.grid.ColumnModel([
        ...
        { header: '类型', tooltip: "类型", width: 80, dataIndex: 'TypeName', sortable: true },
        { header: '角色', tooltip: "角色", width: 80, dataIndex: 'RoleName', sortable: true }
    ]);
    var grid = new Ext.grid.GridPanel({
        ...
        tbar: [
            { text: '增加', iconCls: 'icon-add', hidden: levels.add, handler: function() { url = '/baseset/usertypeadd'; form.getForm().reset(); win.setTitle('增加' + node.text); win.show(); } },
