totolorderdetail = function(node) {
    var frmid = '_' + node.id;
    ...
    var store = new Ext.data.Store({
        proxy: new Ext.data.HttpProxy({
            url: "/report/totolorderdetail/" + totolid,
            method: "POST"
        }),
    ...
    var grid = new Ext.grid.GridPanel({
        ...
        tbar: [
                ...
                { xtype: 'form', method: 'post', id: 'frmDummy' + frmid, action: '/report/exporttotolorderdetail1/' + totolid,
                    target: '_blank', name: 'frmDummy', cls: 'x-hidden', items: [{ xtype: 'textfield', name: 'exportContent', id: 'exportContent' + frmid, type: 'hidden'}]
                },
                {
                    xtype: 'button', name: 'find', text: " &nbsp;导&nbsp;&nbsp;出1&nbsp;&nbsp; ", iconCls: 'icon-excel', pressed: true,
                    listeners: {
                        "click": function() {
                            var vExportContent = grid.getExcelXml();
                            //if (Ext.isIE6 || Ext.isIE7 || Ext.isSafari || Ext.isSafari2 || Ext.isSafari3) {
                            Ext.getCmp('exportContent' + frmid).setValue(vExportContent);
                            var fd = Ext.getCmp('frmDummy' + frmid).getForm().getEl().dom;
                            fd.action = '/report/exporttotolorderdetail1/' + totolid;
                            fd.method = 'post';
                            fd.submit();
                            //} else {
