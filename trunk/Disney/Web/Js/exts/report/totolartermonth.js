totolartermonth = function(node) {
    ...
    var cm = new Ext.grid.ColumnModel([
                ...
                { header: '图片总数', tooltip: "图片总数", width: 80, dataIndex: 'PeopleNum', sortable: true, align: 'right' },
                { header: '总金额', tooltip: "总金额", width: 80, dataIndex: 'Amt', sortable: true, hidden: true, align: 'right' },
    ...
    var grid = new Ext.grid.GridPanel({
        ...
        tbar: [
            { text: '查看统计', iconCls: 'icon-nav', ref: '../totolBtn', disabled: true,
                handler: function() {
                    var s = grid.getSelectionModel().getSelected();
                    var data = s.data;
                    jsload('/js/exts/report/artermonthtotol.js', 'artermonthtotol',
                        { 'id': 'artermonthtotol_' + data.ID, 'text': Ext.util.Format.date(data.BeginTime, 'Y-m-d H:i:s') + '-' + Ext.util.Format.date(data.EndTime, 'Y-m-d H:i:s') + '月结明显'
            ...
            { text: '查看明显', iconCls: 'icon-nav', ref: '../detailBtn', disabled: true,
                handler: function() {
                    var s = grid.getSelectionModel().getSelected();
                    var data = s.data;
                    jsload('/js/exts/report/artermonth.js', 'artermonth',
                        { 'id': 'artermonth_' + data.ID, 'text': Ext.util.Format.date(data.BeginTime, 'Y-m-d H:i:s') + '-' + Ext.util.Format.date(data.EndTime, 'Y-m-d H:i:s') + '月结明显'
                            , 'totolMonth': data, 'attributes': { 'Icon': 'icontab ' + node.attributes.Icon, 'Code': node.attributes.Code }
                        });
                }
            },
