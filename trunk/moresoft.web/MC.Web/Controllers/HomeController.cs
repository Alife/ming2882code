...
namespace MC.Web.Controllers
{
    public class HomeController : BaseController
    {
        ...
        public ActionResult Index()
        {
            ...
            ViewBag.HomePages = _Page_pagServer.GetList(homePageQuery);
            //产品体系
            var productsqi = new QueryInfo();
            productsqi.Parameters.Add("Parent_pag", _Page_pagServer.GetItem("product").ID_pag);
            productsqi.Parameters.Add("top", 3);
            productsqi.Orderby.Add("Sort_pag", null);
            ViewBag.Products = _Page_pagServer.GetList(productsqi).Where(p => !p.Code_pag.Contains("MS-MES"));
            return View();
        }
